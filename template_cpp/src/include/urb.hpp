 ...
 *
 * - Payload carries {origin_id, sequence} as two uint32_t (use the regular PL methods that wraps uint32_t to vector<uint8_t>).
 * - On first reception of a message key (origin, seq), rebroadcast it using PerfectLinks underlying implem.
 * - Deliver once the message has been seen from a majority of processes.
 */
class UniformReliableBroadcast {
public:
    UniformReliableBroadcast(uint8_t process_id, const std::vector<Parser::Host>& hosts, Logger& logger);

    // Set PerfectLinks instance to use for send/rebroadcast
    void setPerfectLinks(PerfectLinks* pl);

    // Broadcast an integer message as URB payload (with origin = self)
    void broadcast(uint32_t message);
    
    // Get the next message sequence to broadcast sequentially (//TODO check if its signal-safe)
...
private:
    /*
    * Message key structure for tracking delivered messages
    * Used to uniquely identify a message by its origin and sequence number
    */
    struct MsgKey {
        uint32_t origin_id;
        uint32_t sequence;
    ...
    /*
     * Hash function for MsgKey to use in unordered_map
     */
    struct MsgKeyHash {
    ...
    Logger& logger_;
    uint32_t majority_threshold_; // floor(N/2) + 1
    ...
    // Synchronization (needed otherwise we have race conditions with signal handler from stress.py)
    mutable std::mutex state_mutex_;
    
    // Garbage collection state (//TODO: does it takes that much memory ?)
